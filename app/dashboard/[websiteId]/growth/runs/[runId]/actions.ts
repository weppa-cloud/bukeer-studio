...
async function getTenantAndRole(websiteId: string) {
  ...
  const { data: website, error: websiteError } = await supabase
    ...
    .eq('id', websiteId)
    .single();
  if (websiteError || !website) throw new Error('Forbidden: tenant guard');

  const accountId = website.account_id as string;
  // TODO(roles): replace with `requireGrowthRole` from auth.ts once #405 lands.
  const ctx = getGrowthRoleStub(accountId, user.id);
  return { supabase, accountId, userId: user.id, role: ctx.role };
}
...
export async function getArtifactSignedUrl(
  ...
  if (error || !row?.artifact_path) {
    return { ok: false, url: null, message: 'No artifact recorded for this run.' };
  }
...
async function writeHumanReview(opts: {
  ...
  const { supabase, accountId, userId, role } = await getTenantAndRole(opts.websiteId);

  if (!hasGrowthRole(role, 'curator')) {
    return {
      ok: false,
      message: 'Forbidden: curator role required (until #405 lands every viewer is `viewer`).',
    };
  }
  ...
  const { error: insertErr } = await supabase.from('growth_human_reviews').insert({
  ...
  if (insertErr) {
    ...
    if (missing) {
      ...
      return {
        ok: false,
        message: 'TODO: growth_human_reviews table not yet provisioned (#405 / #408).',
      };
    }
...
export async function approveRun(formData: FormData): Promise<void> {
  ...
  await writeHumanReview({ websiteId, runId, decision: 'approved' });
}
...
export async function downloadArtifactAction(formData: FormData): Promise<void> {
  const websiteId = String(formData.get('websiteId') ?? '');
  const runId = String(formData.get('runId') ?? '');
  if (!websiteId || !runId) return;
