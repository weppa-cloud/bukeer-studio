...
export function LoginForm({ redirect }: { redirect: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // before the next Server Component request reads auth from cookies.
    if (data.session) {
      writeSupabaseAuthCookie(data.session);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 p-4">
      ...
      >
        ...
        <motion.div
          className={`bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 ${shake ? "animate-shake" : ""}`}
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
          ...
          <div className="mt-3 flex items-center justify-center gap-3 text-sm">
            ...
            <a
              ...
              href={`/demo-code?next=${encodeURIComponent(safeRedirect)}`}
              className="text-blue-600 hover:text-blue-700 dark:text-blue-400"
            >
              Demo code
            </a>
...
function writeSupabaseAuthCookie(session: Session): void {
...
async function waitForSupabaseAuthCookie(): Promise<void> {
