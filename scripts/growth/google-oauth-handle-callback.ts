...
async function main() {
  ...
  console.log(JSON.stringify({
    ...
    callback_state: callbackUrl.searchParams.get('state') ?? null,
    callback_scope: callbackUrl.searchParams.get('scope') ?? null,
...
function parseArgs(argv: string[]) {
  ...
  return {
    ...
    callbackUrl: getString(flags, 'callback-url'),
...
function redact(value: string) {
  return value
    ...
    .replace(/1\/[A-Za-z0-9._-]+/g, '[REDACTED_GOOGLE_REFRESH_TOKEN]')
