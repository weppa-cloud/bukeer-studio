...
const OUT_DIR = `artifacts/seo/${todayIso()}-growth-cache-health`;
...
async function main() {
  ...
  const profileHealth = buildProfileHealth(tables, gscSearchAppearance);
  const report = {
    generated_at: new Date().toISOString(),
    website_id: websiteId,
    status: rollupStatus(tables, gscSearchAppearance),
    max_matrix_status: rollupProfileStatus(profileHealth),
    tables,
    gsc_search_appearance_discovery: gscSearchAppearance,
    max_matrix_profiles: profileHealth,
  };

  await fs.writeFile(
    path.join(OUT_DIR, "growth-cache-health-report.json"),
    JSON.stringify(report, null, 2),
  );
  await fs.writeFile(
    path.join(OUT_DIR, "growth-cache-health-report.md"),
    toMarkdown(report),
  );
    ...
    JSON.stringify(
      {
        ...
        tables: tables.map(
          ({ table, status, row_count, latest_at, age_hours }) => ({
            table,
            status,
            row_count,
            latest_at,
        ...
        max_matrix_status: report.max_matrix_status,
        max_matrix_profiles: summarizeProfileHealth(profileHealth),
      },
      null,
      2,
    ),
...
async function inspectTable(spec) {
  ...
  const timeColumn =
    spec.timeColumns.find((column) => availableColumns.includes(column)) ??
    null;
  const rows = timeColumn
    ? await latestRows(spec.table, availableColumns, timeColumn)
    : { rows: [], error: null };
  const latest = rows.rows[0] ?? null;
  const latestAt = latest ? latest[timeColumn] : null;
  const ageHours = ageInHours(latestAt);
  const expired = availableColumns.includes("expires_at")
    ? await countExpired(spec.table)
...
async function inspectGscSearchAppearance() {
  ...
  if (error) {
    return {
      status: "WATCH",
      implementation:
        "client path added in scripts/seo/populate-growth-google-cache.ts",
      cache_key: null,
      row_count: 0,
  ...
  const standalone = discoveryRows.find((row) =>
    row.cache_key?.includes("|searchAppearance|"),
  );
  const pageCombined = discoveryRows.find((row) =>
    row.cache_key?.includes("page,searchAppearance"),
  );
...
function summarizeRow(row) {
  ...
  return {
    ...
    finding_fingerprint:
      row.finding_fingerprint ?? row.evidence?.finding_fingerprint,
...
function rollupStatus(tables, gscSearchAppearance) {
  if (
    tables.some((table) => table.status === "ERROR" || table.status === "BLOCK")
  )
    return "BLOCK";
  if (
    tables.some((table) => table.status === "EMPTY" || table.status === "WATCH")
  )
    return "WATCH";
...
function buildProfileHealth(tables, gscSearchAppearance) {
  const tableMap = Object.fromEntries(
    tables.map((table) => [table.table, table]),
  );
  return GROWTH_PROVIDER_PROFILES.map((profile) => {
    const cacheStatus = profile.rawCache
      ? (tableMap[profile.rawCache]?.status ?? "UNKNOWN")
      : "n/a";
    const factStatuses = profile.factTargets
      .filter((target) => tableMap[target])
      .map((target) => ({
        table: target,
        status: tableMap[target].status,
        row_count: tableMap[target].row_count,
      }));
    const seed = profileHealthSeed(profile);
    const blockers = [];
    const watches = [];
    ...
    if (profile.status === "planned") {
      blockers.push(
        profile.priority === "P0"
          ? "Profile is P0 but only planned."
          : "Profile is planned.",
      );
    }
    if (
      profile.id === "gsc_search_appearance_v1" &&
      gscSearchAppearance.status !== "PASS"
    ) {
      watches.push("GSC searchAppearance discovery is not PASS.");
    }
    if (profile.rawCache && ["ERROR", "BLOCK", "EMPTY"].includes(cacheStatus)) {
      const message = `Raw/cache ${profile.rawCache} status is ${cacheStatus}.`;
      if (profile.priority === "P0") blockers.push(message);
      else watches.push(message);
    }
    for (const fact of factStatuses) {
      if (["ERROR", "BLOCK", "EMPTY"].includes(fact.status)) {
        const message = `Fact table ${fact.table} status is ${fact.status}.`;
        if (profile.priority === "P0") blockers.push(message);
        else watches.push(message);
      }
    }
    if (requiresApproval(profile)) {
      watches.push(`Approval gate: ${profile.approval}.`);
    }

    const status =
      blockers.length > 0
        ? "BLOCKED"
        : watches.length > 0 || seed === "WATCH"
          ? "WATCH"
          : "PASS";
    return {
      id: profile.id,
...
function rollupProfileStatus(profileHealth) {
  const required = profileHealth.filter((profile) =>
    ["P0", "P1"].includes(profile.priority),
  );
  if (required.some((profile) => profile.status === "BLOCKED"))
    return "BLOCKED";
  if (required.some((profile) => profile.status === "WATCH")) return "WATCH";
  return "PASS";
}

function summarizeProfileHealth(profileHealth) {
  return profileHealth.reduce((acc, profile) => {
...
function toMarkdown(report) {
  const tableRows = report.tables
    .map(
      (row) =>
        `| ${row.table} | ${row.status} | ${row.row_count ?? "n/a"} | ${row.latest_at ?? "n/a"} | ${row.age_hours ?? "n/a"} | ${row.expired_rows ?? "n/a"} | ${row.error ?? ""} |`,
    )
    .join("\n");
  const profileRows = report.max_matrix_profiles
    .map(
      (profile) =>
        `| ${profile.status} | ${profile.priority} | ${profile.provider} | ${profile.id} | ${profile.cadence} | ${profile.implementation_status} | ${profile.cache_status} | ${profile.owner_issues.join(", ")} | ${[...profile.blockers, ...profile.watches].join("<br>") || "No blocker detected"} |`,
    )
    .join("\n");
...
Status: ${report.status}
Max matrix status: ${report.max_matrix_status}
...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    if (arg.startsWith("--website-id="))
      parsed.websiteId = arg.slice("--website-id=".length);
  }
  return parsed;
}
