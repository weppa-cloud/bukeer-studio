...
const outputDir =
  process.env.ADMIN_NEXT_AGENDA_SMOKE_OUTPUT_DIR ||
  'output/playwright/admin-next/agenda';
...
async function main() {
  if (shouldStartServer && !existsSync(buildIdPath)) {
    throw new Error(
      'Missing .next production build. Run `npm run build -- --no-lint` before agenda smoke.',
    );
  }
...
async function startLocalServer() {
  ...
  server.stdout.on('data', (chunk) => {
    process.stdout.write(`[next-agenda:${sessionName}] ${chunk}`);
  });
  server.stderr.on('data', (chunk) => {
    process.stderr.write(`[next-agenda:${sessionName}] ${chunk}`);
  });
...
async function runPlaywrightSmoke(targetUrl) {
  ...
  try {
    ...
    const root = page.locator('[data-testid="admin-next-agenda-root"]');
    await root.waitFor({ timeout: 10_000 });

    const preset = await root.getAttribute('data-theme-preset');
    const title = await page.locator('h1').first().textContent();
    const dayCount = await page.locator('[data-testid^="admin-next-agenda-day-"]').count();
    const serviceCount = await page.locator('[data-testid^="admin-next-agenda-service-"]').count();
    const hasToolbar = await page.getByTestId('admin-next-agenda-toolbar').isVisible();
    const hasAiPanel = await page.getByTestId('admin-next-agenda-ai-panel').isVisible();
    const screenshot = path.join(outputDir, 'agenda-evolucion-light.png');
    await page.screenshot({ path: screenshot, fullPage: false });

    if (preset !== 'evolucion') {
      throw new Error(`Expected Evolucion preset, got ${preset}`);
    }
    if (title?.trim() !== 'Agenda de servicios') {
      throw new Error(`Unexpected agenda title: ${title}`);
    }
    if (dayCount < 3 || serviceCount < 5) {
      throw new Error(`Expected at least 3 days and 5 services, got ${dayCount}/${serviceCount}`);
    }
    if (!hasToolbar || !hasAiPanel) {
      throw new Error('Agenda required panels are not visible.');
    }
