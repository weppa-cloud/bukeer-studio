...
OS. Translation can drive EN-US, Mexico and future locale expansion only when
quality is measurable, reviewable and connected to `growth_inventory`.
...
`seo_translation_qa_findings` stores detailed issues. `growth_inventory` stores
only actionable rows, not every QA observation.
