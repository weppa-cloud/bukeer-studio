...
| `event_id` | `text` UNIQUE NOT NULL | UUIDv4 generated by writer; dedup key globally |
...
Constraints: `UNIQUE (event_id)`, `event_name CHECK` against `event_destination_mapping.event_name`, RLS service-role-only for mutation, read RLS by tenant for reporting.
