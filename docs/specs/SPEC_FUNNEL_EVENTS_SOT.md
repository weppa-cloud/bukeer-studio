...
- [ ] **AC2.2** New conversion actions created in Google Ads UI: `waflow_submit`, `quote_form_submit`, `qualified_lead`, `quote_sent`, `booking_confirmed` (with value_settings = `total_markup`). All marked `primary=true`, `include_in_conversions_metric=true`. **`payment_received` removed from scope** per sign-off 2026-05-03 (Option A — first payment is already covered by `crm_booking_confirmed` via the auto-confirm trigger).
...
- [ ] **AC3.2** `crm_booking_confirmed` event includes `value_amount = itineraries.total_markup` (gross profit, not revenue) and `value_currency = itineraries.currency_type`. Dispatcher fires Meta `Purchase` and Google Ads offline upload `booking_confirmed` with markup as value. **Decision basis (sign-off 2026-05-03)**: ColombiaTours has `total_markup` populated in 98.8% of confirmed itineraries (2,029/2,054 last 12m); identity `total_amount = total_cost + total_markup` exact in 100% of rows; markup avg 21.6%, range 0.3%–100%. Reporting markup as bidding signal lets Smart Bidding optimize for high-margin packages without manual segmentation. Industry consensus for travel with variable margin (Concept, Semetis, Basecamp 2026 guides).
...
- [ ] **AC3.4** ~~DB trigger on `payments` table fires `payment_received` event automatically~~ **REMOVED 2026-05-03 (sign-off Option A)**: `payment_received` would fire at the same instant as `crm_booking_confirmed` (both triggered by first deposit), making it redundant. Cash-flow reporting per `transactions` row is preserved in the CRM/BI layer; not exposed as an Ads conversion event. If multi-installment reporting becomes needed for Ads later, add as a separate spec.
...
3. **Booking value definition** (Phase 3 AC3.2): ~~is "Purchase" the deposit, the full payment, or the contract signed?~~ **RESOLVED + SIGNED OFF 2026-05-03**:
   - **Trigger**: `crm_booking_confirmed` fires when `itineraries.status` transitions to `'Confirmado'` (auto-flips on first deposit via existing trigger `fn_payment_confirms_request`).
   - **Value**: `value_amount = itineraries.total_markup` (gross profit, NOT revenue), `value_currency = itineraries.currency_type`. Industry best-practice for travel with variable margin (avg 21.6%, range 0.3–100% in ColombiaTours data). Lets Smart Bidding auto-optimize for high-margin packages.
   - **Bidding configuration**: target_roas calculated on markup, not revenue. Marketing share of markup: 30–50% per phase → target_roas range **2.0 (aggressive growth) to 3.3 (conservative)**.
   - **`payment_received` event**: REMOVED from scope (Option A). Cash-flow reporting handled in CRM/BI layer.
   - **Refund/cancellation handling**: tracked as separate follow-up issue (`crm_booking_cancelled` event with Meta `Refund` + Google Ads `ConversionAdjustmentService` retraction).
