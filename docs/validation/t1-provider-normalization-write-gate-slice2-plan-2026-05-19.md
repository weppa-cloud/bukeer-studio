# T1 — Write Gate Slice2 Plan / Production DML Preflight
...
- Repo branch clean before work: `feat/growth-source-truth-colombiatours-ptbr` at `431961c5`.
- Candidate selection excluded existing pt-BR seeded entity `/tour-colombia-10-dias`.
- Selected high-evidence pages from existing `es-CO/CO` facts.
- Production DML strategy: idempotent `where not exists`; no `ON CONFLICT` because `growth_source_refs_run_fact_uniq` is deferrable.
...
Proceed with controlled five-entity write only. Do not publish or start transcreation.
