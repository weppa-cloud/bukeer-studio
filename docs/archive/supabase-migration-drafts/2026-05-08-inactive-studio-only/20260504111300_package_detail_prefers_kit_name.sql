  ...
  ELSIF v_product_type = 'package' THEN
    SELECT
      ...
      jsonb_strip_nulls(
        jsonb_build_object(
          'id', i.id,
          'name', COALESCE(NULLIF(pk.name, ''), i.name),
...
comment on function public.get_website_product_page(text, text, text) is
  ...
  || 'closes pilot parity P1 gaps (#250). Package branch now prefers `package_kits.name` over itinerary name and emits `program_gallery` + '
