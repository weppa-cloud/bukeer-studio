...
async function signedRequest(body: Record<string, unknown>, signatureOverride?: string) {
  const raw = JSON.stringify(body);
  const signature = signatureOverride ?? (await hmacHex('chatwoot-secret', raw));
  return {
...
describe('/api/webhooks/chatwoot', () => {
  ...
  it('links a Chatwoot conversation to a WAFlow lead and sends lifecycle conversions', async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      data: {
        matched: true,
        lifecycleEvents: ['ConversationContinued', 'QualifiedLead', 'QuoteSent'],
        conversionsSent: 3,
      },
    });
