...
import type { SeoScoringResult, SeoGrade } from '@/lib/seo/unified-scorer';

interface SeoOverviewTableProps {
  items: SeoScoringResult[];
}
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          value={gradeFilter}
          onChange={(e) => {
            setGradeFilter(e.target.value as SeoGrade | 'all');
            setPage(0);
          }}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--studio-border)] bg-[var(--studio-bg-elevated)]">
              ...
              <th
                className="px-3 py-2 text-left font-medium text-[var(--studio-text-muted)] cursor-pointer select-none"
              ...
              <th
                className="px-3 py-2 text-center font-medium text-[var(--studio-text-muted)] cursor-pointer select-none"
          ...
          <tbody>
            {paginated.length === 0 ? (
              ...
              paginated.map((item) => (
                <tr
                  key={item.id}
                  className="border-b border-[var(--studio-border)] hover:bg-[var(--studio-bg-elevated)] transition-colors"
                >
                  {/* Thumbnail */}
                  ...
                  {/* Name */}
                  <td className="px-3 py-2">
                    <span className="font-medium text-[var(--studio-text)] truncate block max-w-[200px]">
                      {item.name}
                    </span>
                  </td>
                  {/* Type */}
                  ...
                  <td className="px-3 py-2 text-center">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${GRADE_COLORS[item.grade]}`}
                    >
                      {item.grade} <span className="font-normal">({item.score})</span>
                    </span>
                  </td>
                  {/* Issue badges */}
                  <td className="px-3 py-2">
                    <div className="flex flex-wrap gap-1">
                      {item.issues.map((issue) => (
                        <span
                          key={issue}
                          className="inline-block px-1.5 py-0.5 rounded text-[10px] font-medium bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300"
                        >
                          {issue}
                        </span>
                      ))}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page === 0}
              className="px-2 py-1 text-xs rounded border border-[var(--studio-border)] text-[var(--studio-text)] disabled:opacity-40"
            >
              Anterior
            </button>
            <span className="text-xs text-[var(--studio-text-muted)] px-2">
              {page + 1} / {totalPages}
            </span>
            <button
              type="button"
