...
function badgeChip(badge: string, index: number) {
  ...
  return (
    <span key={`${badge}-${index}`} className={className}>
      {isProvider ? (
        <EvoIcon name="building" size={11} />
      ) : isClient ? (
        <EvoIcon name="user" size={11} />
      ) : null}{" "}
      {badge}
    </span>
  );
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <div
          className="searchbox"
          style={{ width: 340 }}
          data-testid="admin-next-contacts-search"
        >
      ...
      <div className="contact-grid" data-testid="admin-next-contacts-grid">
        {fixture.contacts.length > 0 ? (
          fixture.contacts.map((contact) => (
            <Link
              key={contact.id}
              className="card contact-card"
              data-testid={`admin-next-contact-${contact.id}`}
              href={`/admin/contacts/${contact.id}`}
            >
              <div className="top">
                <div className={`av s40 ${TONE_AVATAR[contact.tone] ?? ""}`}>
                  {contact.initials}
                </div>
                <div className="nm">
                  <b>{contact.name}</b>
          ...
          <EvoDataState
            kind="empty"
            title="Sin contactos visibles"
            description="No hay clientes ni proveedores para los filtros actuales."
            actionHref="/admin/contacts"
            actionLabel="Limpiar filtros"
            testId="admin-next-contacts-empty"
          />
        )}
      </div>
    </>
