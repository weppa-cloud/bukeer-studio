  ...
  if (isImmersive) {
    return (
      <div ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden">
        ...
        >
          <span className="font-mono text-[10px] text-white/30 tracking-widest uppercase">{text('sectionScroll')}</span>
          <motion.div
  ...
  return (
    ...
    >
      ...
      <div className="container relative z-10">
        ...
        >
          {(() => {
            const isEditorial = resolveTemplateSet(website) === 'editorial-v1';
            return (
              <>
                <motion.h1
                  initial={{ opacity: 0, y: 20 }}
                  ...
                  className={isEditorial ? "display-xl text-white leading-tight" : "text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-white leading-tight"}
                  dangerouslySetInnerHTML={editorialHtml(title) || { __html: title }}
                />
                ...
                {subtitle && (
                  <motion.p
                    ...
                    className={isEditorial ? "lead text-white/90 max-w-2xl mt-6" : "mt-6 text-lg md:text-xl lg:text-2xl text-white/90 max-w-2xl"}
                    dangerouslySetInnerHTML={editorialHtml(subtitle) || { __html: subtitle }}
                  />
                )}
              </>
            );
