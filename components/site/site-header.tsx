    ...
    <header className={headerClasses[navStyle]}>
      <nav className="container flex items-center justify-between h-16 lg:h-20">
        ...
        <div className="hidden lg:flex items-center gap-8">
                    ...
                    <div className="absolute top-full left-0 pt-2 min-w-48 z-50">
                      <div className="bg-background border rounded-lg shadow-lg py-1">
                        ...
                        {link.children!.map((child) => (
                          <Link
                            key={child.slug}
                            href={resolveNavHref(child, basePath)}
          ...
          {/* Header CTA — configurable or fallback to WhatsApp */}
              ...
              return (
                <div key={link.slug}>
                  <Link
                    ...
                    className="py-3 px-4 rounded-lg text-sm font-medium hover:bg-muted transition-colors block"
                  >
                    {link.label}
                  </Link>
                  {hasChildren && (
                    <div className="pl-6">
                      {link.children!.map((child) => (
                        <Link
                          ...
                          target={child.target === '_blank' ? '_blank' : undefined}
                          className="py-2 px-4 rounded-lg text-sm text-muted-foreground hover:bg-muted transition-colors block"
                        >
                          {child.label}
                        </Link>
                      ))}
...
/**
 * Header CTA button — renders configurable CTA or fallback to WhatsApp
 */
function HeaderCtaButton({
  ...
  // Fallback: WhatsApp if configured
