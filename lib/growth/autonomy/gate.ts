 ...
 *
 * Decision order:
 *   1. observe / prepare are always allowed (no mutation surface).
...
export function evaluateGate(input: GateInput): GateOutput {
  const { actionClass, laneAgreement, policyVersion, smokePass, tenantAutoApplyEnabled } = input;

  // (1) Read-only / staging-only classes have no mutation surface — always allowed.
  if (actionClass === 'observe' || actionClass === 'prepare') {
