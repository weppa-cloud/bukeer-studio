...
async function ensureRequiredRpcsAvailable() {
  if (process.env.E2E_SKIP_RPC_PREFLIGHT === "1") {
    console.log(
      "[globalSetup] RPC preflight skipped via E2E_SKIP_RPC_PREFLIGHT=1",
    );
    return;
  }
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    console.warn(
      "[globalSetup] Skipping RPC preflight (missing Supabase credentials)",
    );
    return;
  }
  ...
  for (const probe of probes) {
    ...
    if (error) {
      console.warn(
        `[globalSetup] RPC probe ${probe.name} returned ${error.code}: ${error.message}`,
      );
      continue;
    }
...
async function globalSetup(config: FullConfig) {
  ...
  if (process.env.E2E_SKIP_ROUTE_PREWARM === "1") {
    console.log(
      "[globalSetup] Route prewarm skipped via E2E_SKIP_ROUTE_PREWARM=1",
    );
    return;
  }
  ...
  for (const route of routesToWarm) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (res.status < 500) {
          console.log(
            `[globalSetup] Warmed ${route} (${res.status}) in ${i + 1}s`,
          );
          warmed = true;
          break;
        }
    ...
    if (!warmed) {
      console.warn(
        `[globalSetup] Warning: ${route} not ready after ${maxAttempts}s — tests may fail`,
      );
    }
  }
}
