...
5. **Patch references**
   - Update internal links `[txt](old/path.md)` → `[txt](new/path.md)` across `docs/`, `.claude/`, `CLAUDE.md`.
...
- Never move `CLAUDE.md`, `AGENTS.md`, `.claude/**`, `packages/**/README.md`.
...
4. **Consistency**
   ...
   - Section-type refs use `SECTION_TYPES` not hardcoded strings (see CLAUDE.md rule).
...
1. Rebuild `docs/INDEX.md` from scratch by scanning `docs/` + `.claude/rules/`.
