...
## Agent entry points               (CLAUDE.md, AGENTS.md, rules, skills, memory)
...
- Wikilink pass: `grep -n "ADR-017" docs/ .claude/` — convert prose refs to `[[ADR-017]]`.
...
1. `grep -rn "\bX\b" docs/ .claude/ CLAUDE.md` (exclude archive/).
...
1. Generate fresh INDEX.md candidate by scanning `docs/`, `.claude/rules/`, and `.claude/skills/*/SKILL.md`.
...
Changes that MUST propagate to `~/.claude/projects/.../memory/project_knowledge_graph.md`:
