...
| New env var in `lib/env.ts` | Update env-vars table in `CLAUDE.md` + relevant ADR/SPEC | — |
...
- Grep `docs/` and `.claude/` for references to the new symbol, path, route name, or env var.
